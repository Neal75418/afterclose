...
void main() {
  ...
  group('syncInvestorConferenceEvents', () {
    late MockTwseClient client2;

    TwseMaterialInfo conf(String code, String rocDate) {
      return TwseMaterialInfo.fromJson({
        ...
        '公司名稱': 'X',
        '主旨 ': '受邀參加法人說明會',
        '說明': '1.召開法人說明會之日期：$rocDate\r\n2.時間：14時',
      });
    }

    setUp(() {
      client2 = MockTwseClient();
      repository = EventRepository(database: mockDb, twseClient: client2);
    ...
    test('法說會公告→EARNINGS 事件（開會日、累積不重複）', () async {
      when(() => client2.getMaterialInformation()).thenAnswer(
        (_) async => [
          conf('1537', '115/07/24'),
          conf('9999', '115/07/25'), // 非自選，略過
        ],
      );
      // 既有事件：同 symbol 同日已存在 → 不重插
      when(
      ...
      expect(captured.single.isAutoGenerated.value, isTrue);
    });

    test('已存在同（symbol,日）事件則跳過（每日檔累積不重複）', () async {
      when(
        () => client2.getMaterialInformation(),
      ).thenAnswer((_) async => [conf('1537', '115/07/24')]);
