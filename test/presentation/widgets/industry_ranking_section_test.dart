...
void main() {
  ...
  Widget buildSection(List<IndustryRanking> rankings) {
    return ProviderScope(
      overrides: [
        industryRankingProvider.overrideWith((ref) async => rankings),
      ],
      child: buildTestApp(
  ...
  const semis = IndustryRanking(
    ...
    topMembers: [
      IndustryMember(symbol: '2330', name: '台積電', ret20Pct: 15.0),
      IndustryMember(symbol: '2454', name: '聯發科', ret20Pct: 10.0),
    ],
  );
  ...
  group('IndustryRankingSection', () {
    testWidgets('顯示產業卡：名稱、動能百分比、名次', (tester) async {
      widenViewport(tester);
      await tester.pumpWidget(buildSection(const [semis, textiles]));
    ...
    testWidgets('點卡片開領漲成員 sheet', (tester) async {
      widenViewport(tester);
      await tester.pumpWidget(buildSection(const [semis]));
      await tester.pump(const Duration(seconds: 1));
