...
void main() {
  ...
  group('InsiderSection', () {
    ...
    testWidgets('shows pledge warning when pledge ratio >= 70', (tester) async {
      widenViewport(tester);
      await pumpSection(
        tester,
        InsiderSection(history: [createEntry(pledgeRatio: 75.0)]),
      );

      expect(find.byIcon(Icons.warning_amber_rounded), findsOneWidget);
    });

    testWidgets('hides pledge warning when pledge ratio < 70', (tester) async {
      widenViewport(tester);
      await pumpSection(
        tester,
