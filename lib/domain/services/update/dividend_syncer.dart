...
class DividendSyncer {
  ...
  /// 各資料來源獨立 try/catch，單一來源失敗不影響其他；股東會事件的全市場重建
  /// 僅在所有已嘗試來源皆成功時執行（見 Phase 4），避免部分失敗誤刪既有資料。
  Future<DividendSyncResult> sync() async {
    var dividendsUpserted = 0;
    var meetingEventsCreated = 0;
    final errors = <String>[];

    // 追蹤各股東會來源本輪是否失敗。Phase 4 的全市場刪除只有在「所有已嘗試的
    ...
    if (_twse != null) {
      try {
        ...
        // TWSE 股東會日期內嵌於已宣告股利回應中——此處失敗代表本輪 TWSE
        // （上市）股東會資料不完整，標記以阻擋 Phase 4 的全市場刪除。
