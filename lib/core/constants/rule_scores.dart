...
abstract final class RuleScores {
  ...
  ///
  /// ⚠️ **市場不對稱**：本規則僅對上市（TWSE）股票生效。專案沒有任何
  /// TPEx 當沖資料來源（`dayTrading` 在 lib/data/remote/ 只出現於
  /// twse_client.dart），day_trading 表全期 0 筆上櫃列，故 900+ 檔上櫃股
  /// 結構性吃不到這 −5。規則本身在 ratio == null 時回 null，不會捏造資料，
  /// 但兩個市場實際競爭在略有差異的評分尺度上。
  ///
  /// 影響量級：2026-06-05 ~ 07-24 期間 TWSE 側僅觸發 39 次（18 檔），
  /// 頻率低但方向系統性。要消除須實作 TPEx 當沖抓取，或改為「無資料
  /// 市場一律不適用」——尚未決定，先如實記錄。
